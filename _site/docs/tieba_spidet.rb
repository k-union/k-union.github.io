...
class Logger<MozillaCookieJarLogger
        ...
        def inspect
                "#<#{self.class.name}: at '#{@cookies_file}'>"
        end
...
class ThreadPost
        ...
        def initialize logger,kz,username,title=nil,summary=nil,reply_num=0,fine=false,top=false,live=false
                ...
                @summary=summary
                @username=username
                @path="/mo/m?kz=#{@kz}&pinf=1_2_0"#后面这个打开管理模式
                @fine=fine
                @top=top
                @live=live
                @reply_num=reply_num
        ...
        def remove
                doc=@logger.webparse @path
                remove_link=doc.at(".//a[text()='删主题']")
                raise TiebaException,"No permission to remove." unless remove_link
                /(\/mo.*)$/===remove_link["href"]
                #check_page=@logger.webparse $1
		#File.write "233.html",check_page.to_html
		check_pg_link=$1
		res=@logger.webget check_pg_link
		check_page=Nokogiri::HTML.parse res.body
                check_link=check_page.at(".//a[text()='确认删除']")
		#warn check_link["href"]
                #/(\/mo.*)$/===check_link["href"]
		#warn $1
                #@logger.webget $1
		/(\/mo.+?)\/m\?/===check_pg_link
		@logger.webget $1+check_link["href"]
                self
        end
end
class Array
